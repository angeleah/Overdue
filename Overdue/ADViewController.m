...
- (void)viewDidLoad
{
    ...
    for (NSDictionary *dictionary in tasksAsPropertyLists) {
        ADTask *taskObject = [self taskObjectForDictionary:dictionary];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    else if ([segue.destinationViewController isKindOfClass:[ADDetailTaskViewViewController class]]) {
        ADDetailTaskViewViewController *detailTaskViewController = segue.destinationViewController;
        NSIndexPath *path = sender;
        ADTask *taskObject = self.taskObjects[path.row];
...
#pragma mark - ADDetailTaskViewControllerDelegate
...
-(ADTask *)taskObjectForDictionary:(NSDictionary *)dictionary
{
    ADTask *taskObject = [[ADTask alloc] initWithData:dictionary];
    return taskObject;
}
...
-(void)updateCompletionOfTask:(ADTask *)task forIndexPath:(NSIndexPath *)indexPath
{
    NSMutableArray *taskObjectsAsPropertyLists = [[[NSUserDefaults standardUserDefaults] arrayForKey:TASK_OBJECTS_KEY] mutableCopy];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ADTask *task = self.taskObjects[indexPath.row];
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        for (ADTask *task in self.taskObjects) {
...
{
    ADTask *taskObject = self.taskObjects[sourceIndexPath.row];
